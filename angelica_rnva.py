...
def download_clarity(path_raw_files,filename, database):
    ...
    my_bcp.dump(query= f'''SELECT loc_name 
                            ...
                            WHERE CAST(order_time AS DATE) >= CAST(DATEADD(day,-7, GETDATE())AS Date) 
                            AND CAST(order_time AS DATE) <= CAST(DATEADD(day,-1, GETDATE())AS Date) 
...
def create_output_file(path_raw_files,filename):
    ...
    with pd.ExcelWriter(f'report_{today}.xlsx') as writer:
        new_df.to_excel(writer, index = None, sheet_name='Weekly_raw_data')
        final_df.to_excel(writer, sheet_name='Report_aggregated_data')
